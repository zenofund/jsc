...
export const sortStaffByHierarchy = (rows: Staff[]) => {
  const categoryPriority = (grade: string) => {
    if (grade === 'CAT4') return 0;
    if (grade === 'CAT1') return 1;
    return 2;
  };
  const getNumericGrade = (grade: string) => {
    if (/^\d+$/.test(grade)) return Number(grade);
    if (/^GL\d+$/i.test(grade)) return Number(grade.replace(/^GL/i, ''));
    return null;
  };
  const toStepNumber = (value: any) => {
    if (typeof value === 'number') return Number.isFinite(value) ? value : 0;
    const digits = String(value || '').match(/\d+/g)?.join('');
    return digits ? Number(digits) : 0;
  };

  return [...rows].sort((a: Staff, b: Staff) => {
    const aGrade = normalizeGrade(a.salary_info.grade_level);
    const bGrade = normalizeGrade(b.salary_info.grade_level);
    const aPriority = categoryPriority(aGrade);
    const bPriority = categoryPriority(bGrade);
    if (aPriority !== bPriority) return aPriority - bPriority;

    const aNum = getNumericGrade(aGrade);
    const bNum = getNumericGrade(bGrade);
    if (aNum !== null || bNum !== null) {
      if (aNum === null) return 1;
      if (bNum === null) return -1;
      if (aNum !== bNum) return bNum - aNum;
      const stepDiff = toStepNumber(b.salary_info.step) - toStepNumber(a.salary_info.step);
    ...
    return toStepNumber(b.salary_info.step) - toStepNumber(a.salary_info.step);
  });
};
...
export function StaffListPage() {
  ...
  const loadStaff = async () => {
    try {
      ...
      const sortedStaff = sortStaffByHierarchy(staffData);
